...
TEST_F(ArenaTest, CreateTest) {
  ...
  ASSERT_EQ(1, a->total_cleanups());  // mock_struct will not be register to cleanup
...
TEST_F(ArenaTest, NullTest) {
  ...
  ASSERT_EQ(y, nullptr);
  ASSERT_EQ(a->space_allocated_, 0ULL);
  ASSERT_EQ(a->total_cleanups(), 0ULL);
