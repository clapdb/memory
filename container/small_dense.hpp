...
template <typename K, typename V> requires (sizeof(K) == 8 && sizeof(V) == 4)
struct k8_v4_bucket {
    ...
    constexpr static bool need_destructor = not (std::is_trivially_destructible_v<K> and std::is_trivially_destructible_v<V>);
    ...
    k8_v4_bucket(uint32_t dist_and_fingerprint, K&& key, V&& value)
        : dist_and_fingerprint(dist_and_fingerprint), value(std::move(value)), key(std::move(key)) {}
    // move constructor
    ...
    k8_v4_bucket(const k8_v4_bucket& other)
        : dist_and_fingerprint(other.dist_and_fingerprint),
          value(other.value),
          key(other.key) {}
...
template <typename K> requires (sizeof(K) == 4)
struct k4_bucket {
    static constexpr uint32_t kFingerWidth = 8U;
    static constexpr uint32_t kDistInc = 1U << kFingerWidth;
    static constexpr uint32_t kFingerprintMask = kDistInc - 1;
    ...
    k4_bucket() = default;
    k4_bucket(uint32_t dist_and_fingerprint, K&& key)
        : dist_and_fingerprint(dist_and_fingerprint), key(std::move(key)) {}
    k4_bucket(const k4_bucket& other)
        : dist_and_fingerprint(other.dist_and_fingerprint), key(other.key) {}
...
template <typename K> requires (sizeof(K) == sizeof(uint64_t))
struct k8_bucket {
    ...
    k8_bucket() = default;
    k8_bucket(uint64_t dist_and_fingerprint, K&& key)
        : dist_and_fingerprint(dist_and_fingerprint), key(std::move(key)) {}
    k8_bucket(const k8_bucket& other)
        : dist_and_fingerprint(other.dist_and_fingerprint), key(other.key) {}
...
template <typename K> requires (sizeof(K) == 3 * sizeof(uint32_t))
struct k12_bucket {
    static constexpr uint32_t kFingerWidth = 8U;
    static constexpr uint32_t kDistInc = 1U << kFingerWidth;
    static constexpr uint32_t kFingerprintMask = kDistInc - 1;
    ...
    k12_bucket() = default;
    k12_bucket(uint32_t dist_and_fingerprint, K&& key)
        : dist_and_fingerprint(dist_and_fingerprint), key(std::move(key)) {}
    k12_bucket(const k12_bucket& other)
        : dist_and_fingerprint(other.dist_and_fingerprint), key(other.key) {}
    k12_bucket(k12_bucket&& other) noexcept
        : dist_and_fingerprint(std::exchange(other.dist_and_fingerprint, 0)), key(std::move(other.key)) {}
}; // struct k12_bucket

template <typename K, typename V> requires (sizeof(K) == 4 && sizeof(V) == 8 && std::is_trivially_copyable_v<K> && std::is_trivially_copyable_v<V>)
struct k4_v8_bucket {
    static constexpr uint32_t kFingerWidth = 8U;
    static constexpr uint32_t kDistInc = 1U << kFingerWidth;
    static constexpr uint32_t kFingerprintMask = kDistInc - 1;
    ...
    k4_v8_bucket(uint32_t dist_and_fingerprint, K&& key, V&& value)
        : dist_and_fingerprint(dist_and_fingerprint), key(std::move(key)), value(std::move(value)) {}
    ...
    k4_v8_bucket(k4_v8_bucket&& other) noexcept
        : dist_and_fingerprint(std::exchange(other.dist_and_fingerprint, 0)), key(std::move(other.key)), value(std::move(other.value)) {}
}; // struct k4_v8_bucket

static_assert(sizeof(k4_v8_bucket<uint32_t, uint64_t>) == 2 * sizeof(uint64_t));

template <typename K, typename V> requires (sizeof(K) == sizeof(uint64_t) and sizeof(V) == sizeof(uint64_t))
struct k8_v8_bucket {
    ...
    k8_v8_bucket(uint64_t dist_and_fingerprint, K&& key, V&& value)
        : dist_and_fingerprint(dist_and_fingerprint), key(std::move(key)), value(std::move(value)) {}
...
template <typename T>
[[gnu::always_inline, nodiscard]] static auto calc_num_buckets_by_shift(uint8_t shift) -> T {
         return T{1} << (64U - shift);
}

template <typename Key, typename Value, class Hash, class KeyEqual, typename Layout = decltype(BucketChooser<Key, Value>()),
          typename BucketContainer = void, float MaxLoadFactor = 0.8F>
class inplace_table
{
    ...
    union bucket_t {
        ...
        bucket_t(): data() {};
        explicit bucket_t(Layout&& layout): layout(std::move(layout)) {}
    ...
    struct default_bucket_container_t
    {
        ...
        bucket_t* end_bucket;
        auto begin() const -> bucket_t* {
            return begin_bucket;
        }
        auto end() const -> bucket_t* {
            return end_bucket;
        }
        auto begin() -> bucket_t* {
            return begin_bucket;
   ...
   public:
    ...
    struct Iterator
    {
        ...
        Iterator(slot_t* current, const slot_t* end) : current(current), end(end) {}
    ...
    private:
     ...
     [[gnu::always_inline]] static void copy_bucket_and_change_fingerprint(const bucket_t* src, bucket_t* dst, distance_and_fingerprint_t dist_and_fingerprint) __attribute__((nonnull)) {
...
#pragma clang loop vectorize(enable)
     ...
     // and the size will just be set, the content of the buckets will be filled later in refill_buckets function
     [[nodiscard, gnu::always_inline]] static auto create_buckets(bucket_index_t new_capacity)
       -> bucket_container_t {
...
#pragma clang loop vectorize(enable)
     ...
     void refill_buckets(bucket_container_t& new_buckets) {
        auto refill_count = _size;
        for (auto iter = _buckets.begin(); refill_count > 0 and iter != _buckets.end(); ++iter) {
            // the filling function will be called in nearly full bucket_container_t, so the bucket is likely to be
            // filled
            if (iter->bucket.dist_and_fingerprint != 0) [[likely]] {
                auto [dist_and_fingerprint, bucket_idx] = next_while_less(iter->bucket.key);
                auto* new_bucket_ptr = new_buckets.at(bucket_idx);
                place_and_shift_up(new_bucket_with_new_dist_and_fingerprint(iter, dist_and_fingerprint),
                                   new_bucket_ptr);
                --refill_count;
            }
        }
     ...
     auto do_rehash_and_emplace_one(uint8_t new_shifts, Key&& new_key, Q&& new_value) -> bucket_t* {
        ...
        return place_and_shift_up(bucket_t{Layout{dist_and_fingerprint, std::forward<Key>(new_key), std::forward<Q>(new_value)}}, bucket_ptr);
     }
     ...
     [[nodiscard]] static constexpr auto at(const bucket_container_t& bucket, bucket_index_t offset)
       -> const bucket_t& {
         return *bucket.at(offset);
     }
     ...
     auto do_place_element(distance_and_fingerprint_t dist_and_fingerprint, bucket_t* place, Key&& key, Q&& value)
       -> bucket_t* {
         if (is_full()) [[unlikely]] {
            ...
            // return place_and_shift_up(bucket_t{Layout{dist_and_fingerprint, std::forward<Key>(key), std::forward<Q>(value)}}, place);
         }
     }
    ...
    template <typename Q = Value, std::enable_if_t<std::is_void_v<Q>, bool> = true>
    auto do_place_element(distance_and_fingerprint_t dist_and_fingerprint, bucket_t* place, Key&& key)
      -> bucket_t* {
        if (is_full()) [[unlikely]] {
            // ignore the place, just place all elements to the new buckets
     ...
     [[nodiscard]] auto do_find(K const& key) -> iterator {
         ...
         // and do not check the dist_and_fingerprint > current_bucket->layout.dist_and_fingerprint, because the dist_and_fingerprint is always increasing.
         // we **hopefully** find the key in the first few buckets.
        if (dist_and_fingerprint == current_bucket->layout.dist_and_fingerprint && _key_eq(key, current_bucket->layout.key)) {
    ...
    auto do_try_emplace(Key&& key) -> std::pair<iterator, bool> {
     ...
     void do_erase(bucket_t* bucket, Op op) __attribute__((nonnull)) {
    ...
    public:
        // constructors
        inplace_table() : _shifts(kInitialShifts) {
            init_buckets_from_shift();
        }

        explicit inplace_table(size_t bucket_count)
            : _shifts(calculate_shifts(bucket_count)) {
            init_buckets_from_shift();
        }
              ...
              _key_eq(other._key_eq) {
                ...
                if constexpr (not Layout::need_destructor) {
                    ...
                    for (auto* other_bucket = other._buckets.begin_bucket, *bucket = _buckets.begin_bucket;
                         _size > 0 and other_bucket != other._buckets.end_bucket;
                         ++other_bucket, ++bucket) {
        ...
        // observers
        [[nodiscard]] auto hash_function() const -> Hash {
            return _hasher;
        }

        [[nodiscard]] auto key_eq() const -> KeyEqual {
            return _key_eq;
        }
        ...
        [[nodiscard, gnu::always_inline]] auto bucket_count() const noexcept -> bucket_index_t { return _buckets.capacity(); }
...
#pragma clang loop unroll(enable)
        ...
        template <class K, class H = Hash, class KE = KeyEqual, std::enable_if_t<detail::is_transparent_v<H, KE>, bool> = true>
        ...
        auto extract(const_iterator pos) -> node_type {
            return extract(iterator{pos.current});
        }
        ...
        template <typename Q = Value, std::enable_if_t<is_map_v<Q>, bool> = true>
        [[nodiscard]] auto at(Key const& key) -> Q& { return do_at(key); }

        template <typename Q = Value, std::enable_if_t<is_map_v<Q>, bool> = true>
        [[nodiscard]] auto at(Key const& key) const -> Q const& { return do_at(key); }

        template <typename Q = Value, std::enable_if_t<is_map_v<Q>, bool> = true>
        [[nodiscard]] auto operator[](Key const& key) -> Q& { return do_at(key); }

        template <typename Q = Value, std::enable_if_t<is_map_v<Q>, bool> = true>
        [[nodiscard]] auto operator[](Key&& key) -> Q& { return do_at(std::forward<Key>(key)); }
        ...
        [[nodiscard]] auto count(Key const& key) const -> size_t {
